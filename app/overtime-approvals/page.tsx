...
import { reviewOvertimeRequestAction } from "@/app/overtime/actions";
...
export default async function OvertimeApprovalsPage({
    ...
    return (
      <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col gap-6 px-6 py-8 md:px-10">
        <section className="rounded-[2rem] border border-dashed border-alert/30 bg-[#fff4ec] p-8 text-sm leading-8 text-alert">
          ...
          SQL truoc khi dung queue duyet tang ca.
        </section>
      </main>
    );
  }
  ...
  const requestsQuery = await supabase
    ...
    .select(
      "id, employee_id, request_date, start_time, end_time, total_hours, reason, status, created_at, review_note",
    )
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-muted md:text-base">
              Queue nay cho manager/admin xem overtime_requests va approve/reject
              ngay trong app.
            </p>
          </div>
      ...
      <section className="grid gap-4">
        {requests.length === 0 ? (
          <article className="rounded-[1.5rem] border border-dashed border-line bg-[#faf7f0] p-6 text-sm leading-7 text-muted">
            Chua co don tang ca nao. Hay vao trang /overtime de tao du lieu test,
            hoac chay seed demo.
          </article>
        ) : (
          requests.map((request) => {
            ...
            return (
              ...
              >
                ...
                <form
                  action={reviewOvertimeRequestAction}
                ...
                >
                  ...
                  <label className="block space-y-2">
                    ...
                    <input
                      name="review_note"
                      type="text"
                      placeholder="VD: Da duyet, bo sung nhan su bao tri."
